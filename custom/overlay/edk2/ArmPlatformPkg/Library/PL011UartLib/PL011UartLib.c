  ...
  )
{
  ...
  if (NumberOfBytes != 0) {
    // The stock path returns as soon as the final byte is queued into the
    // FIFO. On Sky1 this can truncate the last character if firmware
    // immediately reconfigures clocks or hands execution to the next stage.
    // Wait until the FIFO is empty and the transmitter is no longer busy so
    // the custom path always drains complete UART lines before returning.
    while ((MmioRead32 (UartBase + UARTFR) &
            (UART_TX_EMPTY_FLAG_MASK | UART_BUSY_FLAG_MASK)) !=
