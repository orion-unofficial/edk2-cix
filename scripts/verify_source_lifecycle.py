...
def main() -> None:
    ...
    for target in targets:
        ...
        if verbose:
            ...
            print(f"{target}: {len(projections)} overlay projection(s); {summary or 'no overlay paths'}")
        if errors:
            problems.append(f"{target}:")
            problems.extend(f"  - {format_projection(item)}" for item in errors[:20])
            if len(errors) > 20:
                problems.append(f"  ... and {len(errors) - 20} more")

    if problems:
        raise ReconstructionError("source lifecycle validation failed:\n" + "\n".join(problems))

    print(
        f"validated source lifecycle: {len(targets)} target ref(s), "
        f"{total} overlay projection(s), 0 errors in {format_duration(time.monotonic() - started)}"
    )
