...
| Fix | CPU performance: CPPC / `_CPC` | Derive ACPI CPPC nominal and lowest frequency values plus `ReferencePerformance` from SCMI domain attributes and the architectural timer frequency instead of treating SCMI performance levels as MHz or using one fixed reference value for every CPU. | `ENABLE_FIRMWARE_FIXES=true` | `9e29c37d51` |
