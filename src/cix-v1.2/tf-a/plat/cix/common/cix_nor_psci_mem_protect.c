 ...
 */
#define DRAM1_NS_IMAGE_LIMIT  (PLAT_CIX_NS_IMAGE_BASE + (32 << TWO_MB_SHIFT))
#define DRAM1_PROTECTED_SIZE  (SKY1_NS_DRAM1_END + 1u - DRAM1_NS_IMAGE_LIMIT)

static mem_region_t cix_ram_ranges[] = {
	{DRAM1_NS_IMAGE_LIMIT, DRAM1_PROTECTED_SIZE},
#ifdef __aarch64__
	{SKY1_DRAM2_BASE, 1u << ONE_GB_SHIFT},
#endif
};
...
void cix_nor_psci_do_dyn_mem_protect(void)
{
	...
	cix_psci_read_mem_protect(&enable);
	if (enable == 0)
		return;

	INFO("PSCI: Overwriting non secure memory\n");
	clear_map_dyn_mem_regions(cix_ram_ranges,
				  ARRAY_SIZE(cix_ram_ranges),
...
void cix_nor_psci_do_static_mem_protect(void)
{
	...
	(void) cix_psci_read_mem_protect(&enable);
	if (enable == 0)
		return;

	INFO("PSCI: Overwriting non secure memory\n");
	clear_mem_regions(cix_ram_ranges,
			  ARRAY_SIZE(cix_ram_ranges));
	(void) cix_nor_psci_write_mem_protect(0);
}
...
{
	return mem_region_in_array_chk(cix_ram_ranges,
				       ARRAY_SIZE(cix_ram_ranges),
				       base, length);
}
