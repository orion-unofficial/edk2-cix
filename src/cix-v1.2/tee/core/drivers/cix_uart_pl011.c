...
{
	struct cix_pl011_data *pd =
		container_of(chip, struct cix_pl011_data, chip);

	return io_pa_or_va(&pd->base, PL011_REG_SIZE);
}

static void cix_pl011_flush(struct serial_chip *chip)
{
...
static bool cix_pl011_have_rx_data(struct serial_chip *chip)
{
	vaddr_t base = chip_to_base(chip);
...
static int cix_pl011_getchar(struct serial_chip *chip)
{
	vaddr_t base = chip_to_base(chip);

	while (!cix_pl011_have_rx_data(chip))
		;
	return io_read32(base + UART_DR) & 0xff;
}

static void cix_pl011_putc(struct serial_chip *chip, int ch)
{
	vaddr_t base = chip_to_base(chip);
...
static const struct serial_ops pl011_ops = {
	.flush = cix_pl011_flush,
	.getchar = cix_pl011_getchar,
	.have_rx_data = cix_pl011_have_rx_data,
	.putc = cix_pl011_putc,
};
...
static int pl011_dev_init(struct serial_chip *chip, const void *fdt, int offs,
			  const char *parms)
{
	struct cix_pl011_data *pd = container_of(chip, struct cix_pl011_data, chip);
	vaddr_t vbase;
	paddr_t pbase;
	size_t size;
