...
def main() -> int:
    ...
    with tempfile.TemporaryDirectory(prefix=".record-release-", dir=output_root) as tempdir:
        for board, board_config in replay.BOARD_CONFIG.items():
            ...
            for define, env_name in replay.COMMIT_DEFINE_TO_ENV.items():
                commit = summary["build_defines"][define]
                env_values[env_name] = commit
                length_name = (
                    "SOURCE_COMMIT_HASH_LENGTH"
                    if env_name == "SOURCE_COMMIT"
                    else env_name.replace("_SOURCE_COMMIT", "_COMMIT_HASH_LENGTH")
                )
                env_values[length_name] = str(len(commit))
