  ...
  )
{
  ...
  if ((ConfigDesc->Length < sizeof (USB_CONFIG_DESCRIPTOR)) ||
      (ConfigDesc->TotalLength < ConfigDesc->Length)) {
    return EFI_INVALID_PARAMETER;
  }

  MaxDci = 0;

  ConfigDescEnd = (UINT8 *)ConfigDesc + ConfigDesc->TotalLength;
  IfDesc = (USB_INTERFACE_DESCRIPTOR *)((UINTN)ConfigDesc + ConfigDesc->Length);
  for (Index = 0; Index < ConfigDesc->NumInterfaces; Index++) {
    //
    // Ensure the complete interface descriptor is within the configuration.
    //
    if (((UINTN)IfDesc > (UINTN)ConfigDescEnd) ||
        (((UINTN)ConfigDescEnd - (UINTN)IfDesc) < sizeof (USB_INTERFACE_DESCRIPTOR))) {
      goto Done;
    }

    //
    // Bound descriptor traversal even if a malformed chain loops or never
    // reaches the requested interface.
    //
    SkipCount = 0;
