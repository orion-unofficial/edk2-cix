...
[`/.devcontainer/devcontainer.json`](https://github.com/radxa-pkg/edk2-cix/blob/source/unofficial/edk2-stable202602/.devcontainer/devcontainer.json).
...
also passed into the O6 `pm_config` generator unless
`PM_CONFIG_SOURCE_DATE_EPOCH` is set explicitly, so `csu_pm_config.bin`
stops depending on wall-clock time.

`source/unofficial/current` is rebased onto a newer upstream EDK2
...
closest-to-upstream path. On `source/unofficial/current`, the explicit replay
...
`replay.env` plus cert bundle. On `source/unofficial/current`, keep those
...
enough for the Bookworm arm64 path. On `source/unofficial/current`, those
...
`source/unofficial/current`, the following targets are intentionally hidden
...
valid qualification target for `source/unofficial/current` and will fail
